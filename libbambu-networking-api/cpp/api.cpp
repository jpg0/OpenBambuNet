...
#include "open-bambu-net/src/api.rs.h"
...
// Export all API functions with C linkage for OrcaSlicer compatibility
extern "C" {
...
{
    LOG_CALL();
    // Don't call bambu_network_rs_init here - it's called separately from app
...
{
    LOG_CALL_ARGS("%s", selected_device.c_str());
...
{
    LOG_CALL_ARGS("%s", dev_id.c_str());
    std::string url = std::string(bambu_network_rs_get_camera_url(dev_id));
    if (callback) {
        callback(url);
    }
    return 0;
...
// New functions added for newer OrcaSlicer versions
int bambu_network_set_on_subscribe_failure_fn(void *agent_ptr, OnHttpErrorFn fn)
{
    LOG_CALL();
    return 0;
}
...
int bambu_network_add_subscribe(void *agent_ptr, std::string module)
{
    LOG_CALL_ARGS("%s", module.c_str());
    return 0;
}

int bambu_network_del_subscribe(void *agent_ptr, std::string module)
{
    LOG_CALL_ARGS("%s", module.c_str());
    return 0;
}

int bambu_network_enable_multi_machine(void *agent_ptr, bool enable)
{
    LOG_CALL_ARGS("%d", enable);
    return 0;
}

int bambu_network_update_cert(void *agent_ptr, std::string cert_path)
{
    LOG_CALL_ARGS("%s", cert_path.c_str());
    return 0;
}

int bambu_network_install_device_cert(void *agent_ptr, std::string dev_id)
{
    //LOG_CALL_ARGS("%s", dev_id.c_str());
    return 0;
}

int bambu_network_ping_bind(void *agent_ptr)
{
    LOG_CALL();
    return 0;
}

int bambu_network_bind_detect(void *agent_ptr, std::string dev_id)
{
    LOG_CALL_ARGS("%s", dev_id.c_str());
    return 0;
}

int bambu_network_set_server_callback(void *agent_ptr, OnServerConnectedFn connect_fn, OnMessageFn message_fn)
{
    LOG_CALL();
    return 0;
}
...
{
    LOG_CALL_ARGS("%s %s", params.project_name.c_str(), params.filename.c_str());
    return 0;
}

int bambu_network_get_setting_list2(void *agent_ptr, std::string bundle_version, ProgressFn pro_fn, WasCancelledFn cancel_fn)
{
    LOG_CALL_ARGS("%s", bundle_version.c_str());
    return 0;
}

int bambu_network_get_user_tasks(void *agent_ptr, unsigned int *http_code, std::string *http_body)
{
    LOG_CALL();
    *http_code = 500;
    *http_body = "";
...
int bambu_network_get_subtask_info(void *agent_ptr, std::string subtask_id, unsigned int *http_code, std::string *http_body)
{
    LOG_CALL_ARGS("%s", subtask_id.c_str());
    *http_code = 500;
    *http_body = "";
    return 0;
}

int bambu_network_track_remove_files(void *agent_ptr, std::string file_path)
{
    LOG_CALL_ARGS("%s", file_path.c_str());
    return 0;
}

int bambu_network_put_model_mall_rating(void *agent_ptr, std::string model_id, std::string rating_info, unsigned int *http_code, std::string *http_body)
{
    LOG_CALL_ARGS("%s", model_id.c_str());
    *http_code = 500;
    *http_body = "";
    return 0;
}

int bambu_network_get_oss_config(void *agent_ptr, unsigned int *http_code, std::string *http_body)
{
    LOG_CALL();
    *http_code = 500;
    *http_body = "";
...
int bambu_network_put_rating_picture_oss(void *agent_ptr, std::string picture_path, std::string oss_url, unsigned int *http_code)
{
    LOG_CALL_ARGS("%s -> %s", picture_path.c_str(), oss_url.c_str());
    *http_code = 500;
    return 0;
}

int bambu_network_get_model_mall_rating(void *agent_ptr, std::string model_id, unsigned int *http_code, std::string *http_body)
{
    LOG_CALL_ARGS("%s", model_id.c_str());
    *http_code = 500;
    *http_body = "";
...
int bambu_network_get_mw_user_preference(void *agent_ptr, unsigned int *http_code, std::string *http_body)
{
    LOG_CALL();
    *http_code = 500;
    *http_body = "";
...
int bambu_network_get_mw_user_4ulist(void *agent_ptr, unsigned int *http_code, std::string *http_body)
{
    LOG_CALL();
    *http_code = 500;
    *http_body = "";

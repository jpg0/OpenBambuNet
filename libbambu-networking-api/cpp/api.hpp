...
// Forward declarations for legacy types (not used in refactored version)
struct BBLProfile;
struct BBLModelTask;
...
EXPORT int bambu_network_set_on_subscribe_failure_fn(void *agent, OnHttpErrorFn fn);
EXPORT int bambu_network_set_on_user_message_fn(void *agent, OnMessageFn fn);
EXPORT int bambu_network_add_subscribe(void *agent, std::string module);
EXPORT int bambu_network_del_subscribe(void *agent, std::string module);
EXPORT int bambu_network_enable_multi_machine(void *agent, bool enable);
EXPORT int bambu_network_update_cert(void *agent, std::string cert_path);
EXPORT int bambu_network_install_device_cert(void *agent, std::string dev_id);
EXPORT int bambu_network_ping_bind(void *agent);
EXPORT int bambu_network_bind_detect(void *agent, std::string dev_id);
EXPORT int bambu_network_set_server_callback(void *agent, OnServerConnectedFn connect_fn, OnMessageFn message_fn);
...
EXPORT int bambu_network_get_setting_list2(void *agent, std::string bundle_version, ProgressFn pro_fn, WasCancelledFn cancel_fn);
EXPORT int bambu_network_get_user_tasks(void *agent, unsigned int *http_code, std::string *http_body);
EXPORT int bambu_network_get_subtask_info(void *agent, std::string subtask_id, unsigned int *http_code, std::string *http_body);
EXPORT int bambu_network_track_remove_files(void *agent, std::string file_path);
EXPORT int bambu_network_put_model_mall_rating(void *agent, std::string model_id, std::string rating_info, unsigned int *http_code, std::string *http_body);
EXPORT int bambu_network_get_oss_config(void *agent, unsigned int *http_code, std::string *http_body);
EXPORT int bambu_network_put_rating_picture_oss(void *agent, std::string picture_path, std::string oss_url, unsigned int *http_code);
EXPORT int bambu_network_get_model_mall_rating(void *agent, std::string model_id, unsigned int *http_code, std::string *http_body);
EXPORT int bambu_network_get_mw_user_preference(void *agent, unsigned int *http_code, std::string *http_body);
EXPORT int bambu_network_get_mw_user_4ulist(void *agent, unsigned int *http_code, std::string *http_body);
